...
!
!	Marcus A.M. de Aguiar - 18/11/2024
...
! k1,k2,k3 = coupling constants
! delta = width of Gaussian distribution of module of frequencies
! time = integration time
! ifq = type of frequency distribution
...
! Output files
OPEN(UNIT=17,FILE='Figure_3.dat',STATUS='UNKNOWN')

!loops over sigma and force

DO l1=1,101
    sigma = (dfloat(l1)-1.0D0)*0.020D0
    !Random initial conditions on the sphere
    r = 0.0D0
    CALL RANDOM_NUMBER(aux)
    aux0 = aux
    CALL RANDOM_NUMBER(aux)
    ...
    DO i=1,nosc
        aux = aux0
        IF(inicond == 0) CALL RANDOM_NUMBER(aux)
        phi=aux*pi2
        aux = aux1
        IF(inicond == 0) CALL RANDOM_NUMBER(aux)
        ct = 2.0D0*aux - 1.D0  ! cos(theta)
        ...
        r(2) = r(2) + y(j+2)
        r(3) = r(3) + y(j+3)
        ymod = dsqrt(y(j+1)**2 + y(j+2)**2 + y(j+3)**2)
    END DO
    r = r/anosc
    DO l2=1,101
        force = (dfloat(l2)-1.0D0)*0.020D0
        eps=1.D-08
        htry=0.001D0
        ravold = 0.D0
        rsqdold = 0.D0
        ...
        DO WHILE (x < xf)
            DO i=1,n
                yscal(i)=y(i)
                IF(yscal(i) == 0.0d0) yscal(i)=0.01d0
            END DO
            CALL DERIV(X,Y,DY)
        ...
        drmod = rmodsqd/rmod
        dmax = maxval(rsqd)
WRITE(17,'(f7.3,f7.3,f10.7,f10.7,f10.7)')sigma,force,rmodav,drmod,dmax
!        write(6,*) sigma,force,rmod,rmodav,dmax
    END DO
END DO
CLOSE(10)
